...
export const action = async ({ request }: any) => {
  ...
  // const { shop, topic } = response
  // console.log(`Received order_created webhook for ${shop}`);
  // console.log(`Received ${topic} webhook for ${shop}`);

  // const webhookData = JSON.stringify(response, null, 2);
  const orderData = response.payload;
  console.log(`Received order_created webhook for ${orderData}`);

  // return null;

  console.log(`pool.connect begin`);
  const client = await pool.connect(); // 開始交易
  await client.query(`SET search_path TO public;`);
  console.log(`db set path to public`);
  try {
    console.log(`json the orderData begin`);
    // const abc = await request.json(); // 獲取請求內容
    console.log(`json the orderData done`);
...
console.log(`get the dbData out`, orderLineItems);
    ...
    console.log(
      `Processing order for customer: ${customerName}, Email: ${email}`,
    );
    ...
    if (
      orderExists &&
      orderExists.rowCount !== null &&
      orderExists.rowCount > 0
    ) {
    ...
    console.log(
      `Order ${shopifyOrderId} does not exist. Proceeding with insert.`,
    );
    ...
    for (const item of items) {
      ...
      const itemTotalDiscount =
        item.discount_allocations?.reduce(
          (acc: number, discount: { amount: string }) =>
            acc + parseFloat(discount.amount),
          0,
        ) || 0;
